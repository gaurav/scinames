 ...
 */
public class RenamesFromIdsInDataOld implements ChangeGenerator {
	private static final Logger LOGGER = Logger.getLogger(RenamesFromIdsInDataOld.class.getSimpleName());
	
	private DatasetColumn idColumn;
	
	public RenamesFromIdsInDataOld() {
	}
	...
	public Stream<PotentialChange> getRenamesById(Project project, Dataset ds) {
		...
		return identifiersByName.entrySet().stream()
				...
				return project.getRows()
					...
					.flatMap(row -> {
						...
						return row.getDataset().getNamesInRow(row).stream()
							...
							.filter(n -> (n != name))
							.filter(n -> {
								// What is we already know 'n' and 'name' are synonyms?
								// We don't need to add it in here, then, do we?
								NameClusterManager ncm = project.getNameClusterManager();
								
								if(ncm.getCluster(n).equals(ncm.getCluster(name))) return false;
								return true;
							})
							.map(
			...
			.map(syn -> new PotentialChange(syn.getDataset(), ChangeType.RENAME, Stream.of(syn.getFrom()), Stream.of(syn.getTo()), RenamesFromIdsInDataOld.class, syn.getNote()));
	}
}